# Project brief: MyCopilot

## Overview

MyCopilot is a portable GitHub Copilot customization framework that synchronizes custom AI agents, coding instructions, skills, and prompt files across multiple machines via OneDrive. It eliminates the need to manually configure VS Code's Copilot settings on each workstation by storing all customizations in `~/OneDrive/MyCopilot/` and redirecting VS Code to use that location.
