...
VS Code's GitHub Copilot supports custom agents, instructions, skills, and prompt files, but by default these are stored locally in the VS Code profile or workspace. This means customizations do not follow the developer across machines. MyCopilot solves this by redirecting all four customization locations to a OneDrive-synced folder, ensuring every machine gets the same Copilot setup automatically.
...
1. All customization files live in `~/OneDrive/MyCopilot/` organized into four subdirectories: Agents, Instructions, Skills, Prompts.
2. A PowerShell setup script (`Setup-CopilotSettings.ps1`) patches VS Code's `settings.json` to point all four Copilot file-location settings at the OneDrive directory.
...
4. OneDrive sync propagates any changes to all signed-in machines.
