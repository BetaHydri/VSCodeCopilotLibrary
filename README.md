...
| **Instructions** | `*.instructions.md` | Coding standards, conventions, and guidelines. Can auto-apply based on file glob patterns (`applyTo`) or be attached manually. |
...
```jsonc
"chat.agentFilesLocations":        { "~/VSCodeCopilotLibrary/Agents": true }
"chat.instructionsFilesLocations": { "~/VSCodeCopilotLibrary/Instructions": true }
"chat.agentSkillsLocations":       { "~/VSCodeCopilotLibrary/Skills": true }
"chat.promptFilesLocations":       { "~/VSCodeCopilotLibrary/Prompts": true }
```
...
1. Sign into OneDrive so `~/OneDrive/VSCodeCopilotLibrary/` syncs down
2. Open PowerShell and run:

```powershell
& "$env:USERPROFILE\OneDrive\VSCodeCopilotLibrary\Setup-CopilotSettings.ps1"
```
